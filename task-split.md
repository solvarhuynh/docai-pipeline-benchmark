...
Dự án chia thành 2 nhánh công việc song song, phối hợp qua `log/progress-log.md` và JSON schema thống nhất (`shared/schema.py`). Mỗi giai đoạn trong `implementation-guide.md` được tách thành các task cụ thể, gán cho từng nhánh.
...
| Khảo sát thống kê cơ bản (số lượng, độ phân giải, phân bố nhãn) | A |
| Thiết kế JSON schema thống nhất (`shared/schema.py`) | B |
...
| Task | Nhánh phụ trách |
|---|---|
| Chạy pretrained YOLOv8-doc/DocLayout-YOLO, tích hợp pipeline gọi model | A |
| Tinh chỉnh ngưỡng confidence, xử lý edge case layout phức tạp | B |
| Tích hợp PaddleOCR, ghép output vào JSON trung gian | A |
...
| Thiết kế tập fine-tune, xử lý format BIO tagging | B |
...
| Task | Nhánh phụ trách |
|---|---|
| Tích hợp gọi model PaddleOCR-VL/dots.ocr qua API/Modal | A |
...
| Tích hợp cả hai vào `shared/fraud_rules.py`, expose qua API | A |
...
| Vẽ overlay heatmap lên ảnh, đóng gói hàm dùng chung | A |
...
| Phân tích đường cong độ giảm hiệu năng, thiết kế cách đo lường thống kê | B |
...
| Task | Nhánh phụ trách |
|---|---|
| Viết `api/main.py`, expose toàn bộ endpoint | A |
| Deploy Modal, đóng gói Docker cho local | A |
| Chuẩn hoá kiến trúc Dashboard Plotly Dash (`Python → pandas → Plotly → Dash`), không dùng Power BI | A |
...
- Mọi thay đổi JSON schema (`shared/schema.py`) phải thông báo cho nhánh còn lại trước khi merge — vì cả 2 track đều phụ thuộc vào đó.
